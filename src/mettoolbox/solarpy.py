...
import numpy as np
from numpy import arccos, array, cos, deg2rad, exp, rad2deg, sin, tan
...
def sunset_time(date, lat):
    ...
    try:
        ...
        minutes, _ = divmod(aux, 60)
        hours, minutes = divmod(minutes, 60)

        st = datetime(date.year, date.month, date.day) + timedelta(
            hours=(12 + hours), minutes=minutes
        )
        return st
...
def air_mass_kastenyoung1989(theta_z, h, limit=True):
    ...
    if theta_z < 91.5:
        ...
        m = exp(-0.0001184 * h) / (
            cos(theta_z_rad) + 0.50572 * (96.07995 - theta_z) ** (-1.634)
        )
    else:
        theta_z_rad = deg2rad(91.5)
        m = exp(-0.0001184 * h) / (
            cos(theta_z_rad) + 0.50572 * (96.07995 - 91.5) ** (-1.634)
        )
